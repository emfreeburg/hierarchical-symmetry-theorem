...
*Stability* (Theorem 10). If A1 holds only approximately, with residuals bounded by $`\varepsilon`$, then the (tilted, compactified) Lévy measure of the generator is within $`K\sqrt{\varepsilon}`$ of a Dirac mass in the Wasserstein-1 metric, with the constant $`K = ((1+\beta)|\ln r|/|A|)^{1/2}`$ when the limiting increment $`\delta_\infty`$ is known, and the constant $`(2|\ln r|/|A|)^{1/2}`$ when $`\delta_\infty`$ is fitted; both are sharp for the underlying variance bounds.
...
*Beyond independence* (Section 6). For stationary ergodic multipliers, A1 is equivalent to the limiting cumulant generating function—hence the large-deviation rate and the multifractal spectrum—being exactly log-Poisson (Theorem 13); but it provably does not determine the multiplier law: an explicit stationary ergodic countable-state Markov multiplier satisfies A1 exactly with a non-log-Poisson marginal (Theorem 14). Exchangeable multipliers with exact scaling collapse to the i.i.d. log-Poisson cascade (Corollary 15), and finite-state Markov multipliers cannot satisfy the real-$`p`$ form of A1 with nontrivial intermittency (Theorem 16).

*Continuous cascades* (Section 7). In the Bacry–Muzy category of exactly scale-invariant log-infinitely-divisible multifractal random measures, identifiability of the cascade class from structure-function exponents has a sharp finite-order boundary: *four contiguous orders admit impersonators, five exact contiguous orders force the compound Poisson cascade* (Theorem 17); at the level of the scale-invariance generator—magnitude statistics, observable at all orders—A1 selects exactly the Barral–Mandelbrot compound Poisson cascade (Theorem 18), with stability constants that are native and scale-ratio-free (Corollary 19).
...
Throughout this paper, $`\log`$ and $`\ln`$ both denote the natural logarithm.
...
Let $`\Phi(\ell)`$ be the cascade observable at scale $`\ell = r^n`$, operatively $`\Phi(r^n) \stackrel{d}{=} W_1\cdots W_n`$ with $`\Phi(1) = 1`$. Define the *structure functions*
...
*(ii)* Set $`a = \gamma\ln r`$, $`b = (\ln\beta)/k`$, $`\lambda = -C\ln r \ge 0`$; here $`C \ge 0`$ is forced, since the moment sequence $`m \mapsto \mathbb{E}[W^{km}]`$ is log-convex and by ($`\ast\ast`$) its second logarithmic difference is $`C(1-\beta)^2\beta^m|\ln r|`$ ($`C = 0`$ is the degenerate case $`N \equiv 0`$). For $`\log W = a + bN`$ with $`N \sim \mathrm{Poisson}(\lambda)`$:
...
*Step 4* ($`\nu`$ is a single Dirac mass). A1 at $`p = mk`$ gives, by the closed form of Lemma 1, $`\delta_{mk} - \delta_\infty = (\delta_0-\delta_\infty)\beta^m`$; multiplying by $`\ln r`$,
...
Both constants are sharp for the underlying variance bounds from which they arise (and attained in Wasserstein form for $`\beta < 1/2`$; see the remark below). In particular, the cascade multiplier distribution converges to log-Poisson as $`\varepsilon \to 0`$, at the sharp rate $`\Theta(\sqrt{\varepsilon})`$ (Theorems 11 and 12).
...
(a) The variance constant in reading (i) is attained in the limit by the two-atom family $`\eta = c_0\delta_{u_0} + c_1\delta_v`$ with $`u_0 \downarrow 0`$, $`v \in (\beta,1)`$, $`c_1 v(v-\beta) = \varepsilon|\ln r|`$ and $`c_0 = (\varepsilon|\ln r|/\beta)(1 + 1/v)`$: then $`\epsilon_0 \to -\varepsilon`$, $`\epsilon_1 \to +\varepsilon`$, $`|\epsilon_m| = \varepsilon v^{m-1} \le \varepsilon`$ for $`m \ge 2`$, and $`\int(u-\beta)^2 d\eta \to (1+\beta)|\ln r|\varepsilon`$. The Wasserstein constant of the theorem follows by Cauchy–Schwarz; at $`v = 2\beta`$ (feasible for $`\beta < 1/2`$) the family is Cauchy–Schwarz-tight and attains it, while for $`\beta \ge 1/2`$ the Wasserstein form need not be attained.

(b) The variance constant in reading (ii) is likewise attained: choose moment residuals $`(\epsilon_0, \epsilon_1) \to (0, 2\varepsilon)`$ realized by two atoms (one near $`0`$, one in $`(\beta,1)`$) and the offset $`t \to \varepsilon`$; all hypothesis residuals $`h_m = \epsilon_m - t`$ then stay below $`\varepsilon`$ in absolute value. At $`v = 2\beta`$ (again feasible for $`\beta < 1/2`$) this family is likewise Cauchy–Schwarz-tight and attains the Wasserstein constant of reading (ii). For $`\beta < \sqrt{2} - 1`$ one has $`2 > (1+\beta)^2`$: the distinction between the two readings is quantitatively real, and a constant valid in reading (i)—even the non-sharp $`(1+\beta)^2`$—can *fail* outright in reading (ii).
...
```math
W = e^{a_\varepsilon}\prod_i U_i^{1/k}, \qquad \sum_i\bigl(1 - U_i^{1/k}\bigr) \le 2\sum_i(1-U_i) \ \text{ of finite mean } 2\|\eta\|
```
...
with the $`O(\varepsilon)`$ remainders absorbed for $`\varepsilon \le \varepsilon_0`$; at $`k = 1`$ the formalization verifies the sharper constant with $`e^{a_0}`$ in place of $`e^{a_0+1}`$, under the explicit threshold $`\varepsilon_0 = \|\eta\|/(4(1+\beta)|\ln r|)`$ (the formalization's residual is measured at the Lévy–Khintchine scale $`|\ln r|\,\varepsilon`$).
...
Substituting the three displays yields the stated $`K_\infty`$ up to $`O(\varepsilon)`$ remainders, which are absorbed for $`\varepsilon \le \varepsilon_0`$ as in the statement. $`\square`$
...
*(ii)* For $`k=1`$ the conservation drift is $`a_{(d)} = \int(1-u)\,d\tilde\nu_d = \|\eta_d\| = |A|`$, identical to $`a_0 = \lambda(1-\beta) = |A|`$: the family is *drift-rigid*. The support of $`\mathrm{law}(W_0)`$ is the geometric set $`G = \{e^{a_0}\beta^j : j \ge 0\}`$. Take the 1-Lipschitz test function

```math
f(w) := \min\Bigl(\mathrm{dist}\bigl(w,\,G\bigr),\ \tfrac{e^{a_0}\beta(1-\beta)}{2}\Bigr) \;\ge\; 0,
```

which vanishes on $`G`$, so $`\mathbb{E}f(W_0) = 0`$. On the one-jump event of $`W_{(d)}`$ (probability $`\lambda_d e^{-\lambda_d}`$), $`W_{(d)} = e^{a_0}(\beta\pm d)`$, whose distance to the nearest point of $`G`$ is exactly $`e^{a_0} d`$ for $`d < \beta(1-\beta)/2`$ (the neighbors $`e^{a_0}`$ and $`e^{a_0}\beta^2`$ are farther, and $`e^{a_0}d`$ is below the cap); all other events contribute $`\ge 0`$. By Kantorovich–Rubinstein duality [19],

```math
W_1 \;\ge\; \mathbb{E}f(W_{(d)}) - \mathbb{E}f(W_0) \;\ge\; \lambda_d e^{-\lambda_d}\,e^{a_0}\,d,
```

and substituting $`d \ge \bigl(|\ln r|\,\varepsilon(d) / (c_2(\beta)|A|)\bigr)^{1/2}`$ from (i) gives the $`c_1\sqrt{\varepsilon}`$ form. (The same construction works for $`k > 1`$ on the lattice $`\{e^{a_0}\beta^{j/k}\}`$; $`k = 1`$ is stated for cleanliness.) $`\square`$
...
Under (S), A1 holds on the lattice if and only if $`\Lambda = \Lambda_{\mathrm{LP}}`$ on the lattice. Consequently, under A1 every observable computed from lattice exponents—structure-function exponents and moment ratios—coincides exactly with that of the i.i.d. log-Poisson cascade with parameters $`(\beta, \gamma, C)`$; and whenever $`\Lambda = \Lambda_{\mathrm{LP}}`$ holds on all of $`[0,\infty)`$ (as in the construction of Theorem 14), the large-deviation rate function and multifractal spectrum coincide as well (see the remark below; lattice A1 alone pins $`\Lambda`$ off-lattice only up to convexity).
...
(i) $`m_n(p) = r^{n\zeta^{\mathrm{LP}}_p}`$ *exactly*, where $`\zeta^{\mathrm{LP}}_p := \gamma p + C(1-\beta^{p/k})`$ is the log-Poisson exponent curve, for every even $`n`$ and every real $`p \ge 0`$; hence (S) holds, $`\Lambda = \Lambda_{\mathrm{LP}}`$ on all of $`[0,\infty)`$, and A1 holds exactly, in its strongest (real-$`p`$) form;
...
*Stationarity and Markov realization.* The pair $`Z_n := ((n+\theta)\bmod 2,\, X_n)`$ is a Markov chain on $`\{0,1\}\times(a+b\mathbb{N}_0)`$: from phase-0 states the next value is $`a + bA`$ with fresh $`A \sim \mathrm{Poisson}(\lambda')`$ and the phase flips; from phase-1 states the next value is $`a`$ and the phase flips. The chain is irreducible on its reachable set and positive recurrent; the phase-uniform stationary law makes $`(Z_n)`$ stationary and $`W_n`$ a function of it.
...
*Non-log-Poisson marginal.* $`\mathbb{P}(X_1 = a) = \tfrac{1}{2}(1 + e^{-2\lambda})`$, whereas the log-Poisson($`\lambda`$) marginal has $`\mathbb{P}(X = a) = e^{-\lambda}`$; for She–Lévêque dissipation parameters ($`\lambda = 2\ln 2`$): $`0.531`$ versus $`0.250`$. The marginal is the mixture $`\tfrac{1}{2}\delta_a + \tfrac{1}{2}\,\mathrm{law}(a + b\,\mathrm{Poisson}(2\lambda))`$. No choice of parameters repairs this: a log-Poisson marginal charges every point of its lattice $`a' + b'\mathbb{N}_0`$, so support comparison forces $`(a', b') = (a, b)`$; the atom-mass ratios at depths $`j \ge 1`$ are those of $`\mathrm{Poisson}(2\lambda)`$, forcing $`\lambda' = 2\lambda`$, whose top-atom mass $`e^{-2\lambda}`$ lies strictly below the mixture's $`\tfrac{1}{2}(1 + e^{-2\lambda})`$.
...
Let $`\xi`$ be an irreducible finite-state Markov chain, stationary, and $`W_n = e^{f(\xi_n)}`$ with $`f`$ non-constant. Then the cascade cannot satisfy A1 in its real-$`p`$ form—the closed form ($`\ast\ast`$) valid for all real $`p \ge 0`$—with nontrivial intermittency: there are no parameters $`(\beta, \gamma, C)`$ with $`C > 0`$ and $`\zeta_p = \gamma p + C(1-\beta^{p/k})`$ for all real $`p \ge 0`$.
...
and moments of the total mass are finite (for $`q > 1`$) precisely on the window where $`\zeta_q > 1`$ [2]. Which orders are usable thus depends on the generator: when $`\zeta_q`$ eventually decreases (log-normal, log-stable) only a finite window $`[0, q^\ast)`$ of finite moments exists, whereas the She–Lévêque curve itself is strictly increasing under (M2) whenever $`\gamma \ge 0`$ (at She–Lévêque values $`\gamma = 1/3`$), so every moment of the compound Poisson cascade is finite. The operative limit on structure-function data is statistical: high-order sample moments are noise-dominated, and in practice exponents are trusted only on a short initial window, typically $`q \lesssim 3`$. The hierarchical symmetry A1 is imposed on $`\delta_q = \zeta_{q+k} - \zeta_q`$ as before.
...
### Theorem 17 (Four orders do not identify the class; five contiguous orders do)

Fix the She–Lévêque exponent curve $`\zeta^{\mathrm{SL}}_q = \gamma q + C(1-\beta^{q/k})`$, $`\beta \in (0,1)`$, $`C > 0`$, normalized by (M2).

**(i) (Impersonation up to four orders.)** Assume additionally $`\gamma \ge 0`$, as at She–Lévêque values, so that the matched curve keeps the measures nondegenerate (every impersonator below matches $`\zeta_{2k} = \zeta^{\mathrm{SL}}_{2k} > 1`$). For every lattice window $`F \subseteq \{0, k, 2k, 3k\}`$ there is a continuum of exactly scale-invariant log-ID multifractal random measures whose generators are not compound Poisson with a single atom—in particular are not the compound Poisson cascade—yet whose exponents satisfy $`\zeta_q = \zeta^{\mathrm{SL}}_q`$ for every $`q \in F`$. Moreover every impersonator of the full window $`\{0,k,2k,3k\}`$ is strictly *more* intermittent at the next order: $`\zeta_{4k} < \zeta^{\mathrm{SL}}_{4k}`$, with $`\zeta^{\mathrm{SL}}_{4k} - \zeta_{4k} \le C\beta(1-\beta)^3`$ for generators without amplifying jumps.

**(ii) (Five-point rigidity.)** Let $`M`$ satisfy (M1), (M2) with generator triplet $`(\tilde a, \sigma_0^2, \Pi)`$ and Lévy exponent $`\psi`$ finite on $`[0, 4k]`$ (weaker than (S<sub>c</sub>)). If $`\zeta_q = \zeta^{\mathrm{SL}}_q`$ at the five contiguous points $`q \in \{0, k, 2k, 3k, 4k\}`$—in particular on any window $`F \supseteq \{0, k, 2k, 3k, 4k\}`$—then $`\sigma_0^2 = 0`$ and $`\Pi = C\delta_b`$ with $`b = (\ln\beta)/k`$: $`M`$ *is* the compound Poisson cascade. No impersonator exists.

Structure-function data on the practically accessible window (typically $`q \lesssim 3`$: part (i)) cannot certify the log-Poisson class; five exact contiguous exponent values (part (ii)) can.

*Proof.* Throughout take $`k = 1`$ (general $`k`$ is identical after $`u = e^{kx}`$) and work in the tilted coordinates $`u = e^{x}`$, in which the CPC generator (Theorem 18) has $`\tilde\Pi = C\delta_\beta`$. The key object is the *squared-tilt* measure

```math
\rho \;:=\; (u-1)^2\,d\tilde\Pi \;\ge\; 0 \quad\text{on } (0,\infty)\setminus\{1\}, \qquad r_q := \int u^q\,d\rho\,;
```

because $`(u-1)^2`$ is a square, $`\rho`$ is nonnegative on *both* sides of $`u = 1`$. Finiteness of $`r_0, r_1, r_2`$ follows from $`\psi(4) < \infty`$: near $`x = 0`$ the integrand is $`\asymp x^2 u^q`$ (Lévy integrability); on $`x \le -1`$ it is bounded; on $`x \ge 1`$ it is $`\le e^{(q+2)x}`$ with $`q + 2 \le 4`$. Taking the step-one second difference of the Lévy–Khintchine formula, the drift and compensator terms cancel exactly, the Gaussian term contributes $`\sigma_0^2`$, and $`e^{(m+2)x} - 2e^{(m+1)x} + e^{mx} = e^{mx}(e^{x}-1)^2`$, so

```math
\Delta^2\psi(m) \;=\; \psi(m+2) - 2\psi(m+1) + \psi(m) \;=\; \sigma_0^2 + r_m, \tag{3}
```

while on the She–Lévêque side $`\Delta^2\psi^{\mathrm{SL}}(m) = C\beta^m(1-\beta)^2 =: A'\beta^m`$.

*(ii), five-point rigidity.* Matching $`\zeta`$ (hence $`\psi`$) at the five contiguous points $`\{0,1,2,3,4\}`$ matches the three second differences $`m = 0, 1, 2`$ of (3): $`\sigma_0^2 + r_m = A'\beta^m`$. Test against the square $`(u-\beta)^2`$:

```math
0 \;\le\; \int (u-\beta)^2\,d\rho \;=\; r_2 - 2\beta r_1 + \beta^2 r_0 \;=\; A'(\beta^2 - 2\beta^2 + \beta^2) - \sigma_0^2(1 - 2\beta + \beta^2) \;=\; -\sigma_0^2(1-\beta)^2 \;\le\; 0.
```

Hence $`\sigma_0^2 = 0`$ and $`\int(u-\beta)^2\,d\rho = 0`$, so $`\rho`$ vanishes off $`\{\beta\}`$ and $`\rho = r_0\delta_\beta = A'\delta_\beta`$. Inverting the tilt ($`(u-1)^2 > 0`$ off $`u = 1`$, and $`\tilde\Pi(\{1\}) = \Pi(\{0\}) = 0`$ by the Lévy convention) gives $`\tilde\Pi = A'(1-\beta)^{-2}\delta_\beta = C\delta_\beta`$, i.e. $`\Pi = C\delta_b`$; (M2) pins $`\tilde a`$ and $`\psi \equiv \psi^{\mathrm{SL}}`$. The identification with the Barral–Mandelbrot cascade is [2]. Note no divergence argument and no moments beyond $`q = 4`$ are used: the squared tilt is sign-definite across $`u = 1`$, so amplifying jumps ($`u > 1`$) and the Gaussian component die on the finite window.

*(i), construction at four orders.* For the physically typical window $`F = \{0,1,2\}`$ perturb the CPC:

```math
\tilde\Pi_s = (C - sc)\,\delta_\beta + s\,(w_1\delta_{v_1} + w_2\delta_{v_2}), \qquad 0 < v_1 < \beta < v_2 < 1,
```

with the drift re-fixed by (M2) for each $`s`$, and impose $`\sum_{i} w_i(v_i^m - 1) = c(\beta^m - 1)`$ for $`m = 1, 2`$: the $`m=1`$ equation makes the (M2)-drifts coincide, the $`m=2`$ equation matches $`\zeta_2`$, and $`\zeta_0 = 0`$, $`\zeta_1 = 1`$ are automatic. At $`(\beta, v_1, v_2) = (2/3,\,0.3,\,0.9)`$, $`c = 1`$, the $`2\times2`$ system gives $`w_1 = 5/27 \approx 0.1852`$, $`w_2 = 55/27 \approx 2.0370`$, both *positive*, so $`\tilde\Pi_s \ge 0`$ for all $`s \in [0, C/c]`$: a one-parameter family of genuine Lévy measures, none a single atom for $`s > 0`$, all matching $`\zeta^{\mathrm{SL}}`$ exactly on $`F`$ (and differing beyond: at $`m = 3`$ the perturbed exponent differs by $`+77/5400 \approx +0.0143`$ at $`s = \tfrac{1}{2}`$).

This two-sided ansatz, however, does *not* extend past $`F = \{0,1,2\}`$: if $`\mu^+ = \sum_i w_i\delta_{v_i} \ge 0`$ with $`v_i \in (0,1)`$ satisfies the constraints for $`m = 1, 2, 3`$, then with $`d\sigma := (1-u)\,d\mu^+ \ge 0`$ and $`1 - u^m = (1-u)(1 + u + \cdots + u^{m-1})`$ the constraints give $`s_j := \int u^j d\sigma = c(1-\beta)\beta^j`$ for $`j = 0, 1, 2`$, whence $`\int(u-\beta)^2 d\sigma = 0`$ and $`\mu^+ = c\,\delta_\beta`$: the perturbation collapses to the CPC atom. The maximal window $`\{0,1,2,3\}`$ is reached instead by a one-sided construction in $`\rho`$-space: solve $`r_0 = A'`$, $`r_1 = A'\beta`$ with two atoms, e.g. $`\rho = w_1\delta_{0.4} + w_2\delta_{0.95}`$, $`(w_1, w_2) = (0.114478,\,0.107744)`$ at $`(\beta, C) = (2/3, 2)`$, i.e. $`\tilde\Pi = \rho/(1-u)^2 = 0.31799\,\delta_{0.4} + 43.0976\,\delta_{0.95}`$ with (M2)-drift $`\tilde a = 2.345679`$. By (3) this matches $`\zeta^{\mathrm{SL}}`$ at $`q = 0, 1, 2, 3`$ exactly (with $`\zeta_2 = \tfrac{16}{9} > 1`$: a nondegenerate multifractal measure), is not single-atom compound Poisson, and deviates at $`q = 4`$ by $`-(r_2 - A'\beta^2) = -0.01679`$. Varying the atom positions $`0 < v_1 < \beta < v_2 < 1`$ solves the same two constraints with strictly positive weights $`w_1 = A'(v_2-\beta)/(v_2-v_1)`$, $`w_2 = A'(\beta-v_1)/(v_2-v_1)`$, giving a two-parameter continuum of impersonators of the full window.

*(i), one-sidedness at the next order.* For *any* generator matching $`\zeta^{\mathrm{SL}}`$ on $`\{0,1,2,3\}`$, (3) at $`m = 0, 1`$ gives $`\sigma_0^2 + r_0 = A'`$ and $`\sigma_0^2 + r_1 = A'\beta`$, and

```math
\zeta_{4} - \zeta^{\mathrm{SL}}_{4} = -\bigl(\sigma_0^2 + r_2 - A'\beta^2\bigr).
```

Cauchy–Schwarz $`r_1^2 \le r_0 r_2`$ forces $`\sigma_0^2 + r_2 \ge A'\beta^2`$, with equality iff $`\sigma_0^2 = 0`$ and $`\rho = A'\delta_\beta`$—i.e. iff the generator is the CPC itself. Hence every genuine impersonator strictly undershoots at $`q = 4`$; for attenuation-only generators (no atoms $`u > 1`$) the deficit is at most $`A'\beta(1-\beta) = C\beta(1-\beta)^3`$ ($`= 4/81 \approx 0.049`$ at She–Lévêque values), while amplifying atoms make it unbounded as $`u_{\max} \to \infty`$. $`\square`$

**Remark.** The divergence steps of Theorem 7 need $`q \to \infty`$, and on a three-point window even $`\sigma_0^2 > 0`$ survives (a quadratic log-normal exponent interpolates any three-point window with the correct convexity)—but the obstruction stops there. The squared tilt $`\rho = (u-1)^2 d\tilde\Pi`$ of part (ii) is sign-definite across $`u = 1`$, so the Gaussian component and amplifying jumps die on a finite window with no divergence bookkeeping: identifiability does *not* require constraints of unbounded order—five exact contiguous orders suffice. (Version 2 of this paper asserted the contrary; part (ii) corrects it.) What structure functions cannot supply in practice is exactness at five orders inside the statistically accessible window, typically $`q \lesssim 3`$: the barrier to certification by structure functions is statistical, not one of principle. The scale-invariance factor $`\Omega_\sigma`$ remains the robust observable: under (S<sub>c</sub>) it has all exponential moments, at every $`\sigma`$, and its statistics ("magnitude" statistics, in the language of [5]) are observable at all orders.
...
**Remark (What the two theorems mean together).** Theorems 17 and 18 are two halves of one statement about observability: on the practically accessible window (typically $`q \lesssim 3`$) structure functions cannot identify the cascade class, while five exact contiguous orders—or, at the generator level, magnitude statistics at three lattice orders—classify it completely, with quantitative stability. This places a theorem under the long-standing practical preference for magnitude-cumulant analysis over high-order structure functions [5], now as a statement about statistics rather than principle: the structure-function route fails not because finite-order data can never certify the class, but because exactness at the five orders it would need is beyond finite-sample resolution. Note also that the continuous category is *more* rigid than the discrete one: infinite divisibility, an assumption in Theorem 7, is automatic here (consistency of $`\Omega_{\sigma\sigma'} \stackrel{d}{=} \Omega_\sigma + \Omega'_{\sigma'}`$), so the classification needs no distributional hypothesis beyond membership in the class—and by Theorem 17(ii) it needs only five lattice moments, with no divergence bookkeeping. Finally, the most-singular-branch geometry (Corollary 4) reads natively: the probability that the cone above a point carries no Poisson point down to scale $`\ell`$ is $`\ell^{\,C}`$—codimension $`C`$, with no discretization anywhere.
...
**Corollary 21 (Codimension identification).** If the most singular structures have Hausdorff codimension $`C_{\mathrm{geom}}`$ and $`C = C_{\mathrm{geom}}`$, then, together with the conservation constraint of Corollary 20, $`\beta`$ alone determines the full exponent curve, the multifractal spectrum, and the cascade distribution.
...
4. **Statistics of the A1 test.** The present results are exact-population statements. A finite-sample theory—error bars on $`(\hat\beta, \hat d^\ast)`$, power against log-normal and log-stable alternatives, with the reading-(ii) constants of Theorem 10 and Corollary 19 as the operative null band—is the missing link between the classification and data; the window obstruction of Theorem 17(i) dictates that such a theory be built on magnitude statistics rather than high-order structure functions. (By Theorem 17(ii) this is a dictate of statistics, not principle: five exact contiguous orders would suffice, but exactness at five orders is precisely what finite noisy data cannot supply.) Sparse lattice windows—neither contained in $`\{0, k, 2k, 3k\}`$ nor containing $`\{0, k, 2k, 3k, 4k\}`$—are not settled by Theorem 17 and remain open.

---

## Appendix A. Formalization Status

The mathematical core of this paper is formalized and kernel-checked in Lean 4 with Mathlib (repository directory [`formalization/`](formalization/), sixteen modules): **zero `sorry`, zero custom axioms** — every compiled constant depends only on Lean's three standard axioms (`propext`, `Classical.choice`, `Quot.sound`), enforced by a whole-library environment walk ([`AxiomAudit.lean`](formalization/AxiomAudit.lean)) that fails on any violation and thereby also soundly excludes every escape hatch (`sorry` introduces the axiom `sorryAx`). Toolchain and Mathlib revision are pinned; `./check.sh` reproduces the full gate, and [`Freeburg/Main.lean`](formalization/Freeburg/Main.lean) is a compile-checked manifest of the principal claims (it fails to build if any is renamed or removed).

**Machine-checked** (Lean names in the [repository's mapping tables](formalization/README.md)): Lemma 1, with the lattice-level skeleton of Corollary 6 (`satisfiesA1_iff_closed_form`); Proposition 5 (`logPoisson_satisfies_A1`); Lemma 2 (`lemma2_boundedness`, `ext_of_lattice_moments_eq`); Theorem 3(ii) at the level of laws (`theorem3_law_uniqueness_W`); Theorem 7, forward (`theorem7_forward_nu`); Theorem 10 under both readings with both sharp constants attained exactly (`theorem10_variance_known/_fitted`, `variance_le_sharp`, `fitted_variance_le_sharp`, Wasserstein form `theorem10_wasserstein_W1`); Theorem 11, unconditional (`theorem11_rate_upper`); Theorem 12 (`theorem12_rate_lower`); the Θ(√ε) bracket (`theorem11_12_theta`); and, in this version, Theorem 17 in both parts — five-point rigidity (`theorem17_five_point_rigidity`), one-sidedness with its equality case and deficit budget (`theorem17_one_sided`, `theorem17_one_sided_budget`), and the exact-rational impersonators including the injective three-atom continuum (`theorem17_impersonation_three_point`, `nu3_injOn`, `theorem17_impersonation_four_point`) — together with Theorem 18's forward direction by the five-moment route under hypotheses weaker than (S<sub>c</sub>) (`theorem18_forward_fiveMoment`). Section 6 is checked in scoped form: Theorem 13 in full (`theorem13_asymptotic_classification`); Theorem 14's quantitative core — exact even-block log-Poisson moments and the strict marginal mismatch $`\tfrac{1}{2}(1+e^{-2\lambda}) > e^{-\lambda}`$ (`theorem14_even_block_moments`, `theorem14_marginal_ne_logPoisson`); Corollary 15 in conditional form — the de Finetti directing kernel as hypothesis, after which variance-vanishing and pointwise determinacy are fully checked (`corollary15_conditional`); and Theorem 16's growth-contradiction core (`theorem16_scoped`). Corollaries 19–23 are checked (`corollary20_conservation` through `corollary23_native_stability`, the latter carrying Corollaries 19 and 23 as a single statement, with the spectrum width of Corollary 22 obtained from a kernel-checked derivative).

**Strengthenings found by formalization** (each documented in the repository): the converse needs no $`\lambda > 0`$; the Dirac rigidity is uniform in $`\beta`$ and needs no sign hypothesis on the intensity ($`C \ge 0`$ is a conclusion); the variance-level sharp constants are attained exactly on the closed interval, not only in the limit; Theorem 12(ii) holds for every target rate and drift; Theorem 11's constant improves at $`k = 1`$ and never consumes conservation; the one-sided deficit budget needs no $`\sigma^2 \ge 0`$. The formalization also caught one sign typo (Theorem 12's parenthetical, fixed before submission of v2).

**Still informal, and why:** the classical inputs that the formalized statements deliberately take as hypotheses rather than re-prove, pending Mathlib infrastructure: the Lévy–Khintchine representation theorem (the log-ID class is formalized *by triplet*, exactly as Theorems 7, 10, 17 and 18 state their hypotheses); de Finetti's theorem (Corollary 15's directing kernel); Perron–Frobenius simplicity with analytic perturbation and the identity theorem (Theorem 16's extension of $`\Lambda`$ to $`\mathbb{R}`$); the Bacry–Muzy construction of the exactly scale-invariant measure for a given generator (§7 results are stated at the generator level, as their printed proofs argue); and Theorem 14's stationary-ergodic sequence-space realization together with the Gärtner–Ellis remark (its block-level moments and marginal mismatch are checked). Minor items riding on these bridges (Theorem 3(iii), Corollaries 4 and 8, Proposition 9(b)) and the sparse-window question remain informal. One definitional dictionary is likewise classical: the formalization states $`W_1`$ in Kantorovich–Rubinstein dual form, whose equality with transport $`W_1`$ on compact supports is classical duality (lower bounds transfer unconditionally). The boundary between machine-checked and informal is exactly as stated here; no other unformalized input is used.
...
8. B. Dubrulle and F. Graner, Possible statistics of scale invariant systems, *J. Phys. II France* **6** (1996), 797–816.
...
17. Z.-S. She and E. C. Waymire, Quantized energy cascade and log-Poisson statistics in fully developed turbulence, *Phys. Rev. Lett.* **74** (1995), 262–265.
